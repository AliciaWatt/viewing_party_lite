...
RSpec.describe 'new party page' do
  let!(:user1) { create(:user, name: 'person1', email: 'email1@email1.com') }
  let!(:user2) { create(:user, name: 'person2', email: 'email1@email2.com') }
  let!(:user3) { create(:user, name: 'person3', email: 'email1@email3.com') }
  let!(:user4) { create(:user, name: 'person4', email: 'email1@email4.com') }
  before(:each) do
