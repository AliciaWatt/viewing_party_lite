 ...
 describe 'top 20 movies' do 
  ...
  describe 'search happy path' do
    it 'can search by keyword and return relavant video if exists' do
      VCR.use_cassette('an search by keyword and return relavant video if exists') do
  ...
  describe 'search sad path' do
    it 'returns error message if no keyword match', :vcr do
        VCR.use_cassette('an search by keyword and return relavant video if exists') do
        fill_in :keyword, with: 'xwlijg'
        click_button 'Search'

        expect(current_path).to eq("/users/#{user1.id}/discover")
        expect(page).to have_content("Error: No movies found containing 'xwlijg', please try again!.")
      end
    end
  end
end
